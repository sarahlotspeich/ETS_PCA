...
SRS_results |> 
  ...
  dplyr::left_join(data.frame(Model = paste0("X", 1:5), 
                              Truth = beta1)) |> 
  write.csv(file = paste0("mi_indep_covar_", sim_seed, ".csv"), 
            row.names = FALSE)
...
SRS_results |> 
  ...
  dplyr::left_join(data.frame(Model = paste0("X", 1:5), 
                              Truth = beta1)) |> 
  write.csv(file = paste0("mi_dep_covar_equal_", sim_seed, ".csv"), 
            row.names = FALSE)
...
SRS_results |> 
  ...
  dplyr::left_join(data.frame(Model = paste0("X", 1:5), 
                              Truth = beta1)) |> 
  write.csv(file = paste0("mi_dep_covar_unequal_", sim_seed, ".csv"), 
            row.names = FALSE)